...
**문서 갱신 시각: 2026-02-12 02:14:44 KST**
**기준 경로: `/mnt/c/Users/USER/growit-pipeline`**

이 문서는 코드, 로그, 데이터 파일 스냅샷 기준으로 현재 상태를 상세 기록한 문서입니다.

## 오늘 추가 업데이트 (2026-02-12)

프론트 인증/접근 흐름과 학습 현황 표시 위치를 아래와 같이 최신화했습니다.

- Supabase 환경 변수 키 추가
  - `growit-pipeline/.env`
  - `daily-spark-learn-main/daily-spark-learn-main/.env`
  - 추가 키: `VITE_SUPABASE_URL`, `VITE_SUPABASE_ANON_KEY`
- Supabase 클라이언트 파일 추가
  - `daily-spark-learn-main/daily-spark-learn-main/src/lib/supabaseClient.ts`
- 로그인 UI 로직 주입(기존 Lovable UI 유지)
  - `daily-spark-learn-main/daily-spark-learn-main/src/pages/Auth.tsx`
  - `supabase.auth.signInWithOtp({ email })` 기반 매직링크 로그인 적용
- 앱 진입 장벽 제거 + 세션 상태 공유 컨텍스트 도입
  - `daily-spark-learn-main/daily-spark-learn-main/src/App.tsx`
  - `daily-spark-learn-main/daily-spark-learn-main/src/contexts/AuthContext.tsx` (신규)
  - 결과: 비로그인 사용자도 메인 앱 진입 가능, 세션 상태는 하위에서 공통 사용
- 헤더 로그인/로그아웃 버튼 전환 적용
  - `daily-spark-learn-main/daily-spark-learn-main/src/components/Header.tsx`
  - 비로그인: 로그인 버튼(`/auth`)
  - 로그인: 로그아웃 버튼(`supabase.auth.signOut()`)
- 오늘의 학습 위치 이동
  - 기존: `Index` 본문 상단 카드
  - 변경: `Header` 우측 영역(아이콘 오른쪽) 상시 표시
  - 로그인 사용자에 한해 `오늘 학습 시간/연속 학습` 표시, `last_study_date != 오늘`이면 `오늘 학습 시간=0분` 처리
  - 관련 파일: `daily-spark-learn-main/daily-spark-learn-main/src/components/Header.tsx`, `daily-spark-learn-main/daily-spark-learn-main/src/pages/Index.tsx`
